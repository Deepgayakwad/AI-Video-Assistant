...
/* ── Markdown rendered inside bordered result containers ── */
...
[data-testid="stVerticalBlockBorderWrapper"] strong { color: var(--accent-glow) !important; }
[data-testid="stVerticalBlockBorderWrapper"] a { color: var(--accent-2) !important; }
[data-testid="stVerticalBlockBorderWrapper"] hr { margin: 0.8rem 0 !important; }
...
.chat-bubble { display: inline-block; padding: 0.6rem 1rem; border-radius: 10px; font-size: 0.85rem; line-height: 1.6; max-width: 90%; white-space: pre-wrap; word-break: break-word; }
...
def safe_html(text: str) -> str:
    """Escape user/LLM-generated text before injecting into raw HTML, preserving line breaks."""
    if text is None:
        return ""
...
def render_card(title: str, icon: str, content: str):
    """Render a titled card using a real bordered container so the content
    is passed through st.markdown() and actual Markdown (##, **bold**,
    bullet lists, links) renders properly instead of showing as raw text."""
    with st.container(border=True):
        st.markdown(
            f'<div class="card-title">{icon} {title}</div>',
            unsafe_allow_html=True,
        )
        st.markdown(content if content else "_No content returned._")
...
with st.sidebar:
    ...
    if st.session_state.pipeline_done:
        st.markdown("---")
        st.markdown('<span class="badge badge-green">Last Run</span>', unsafe_allow_html=True)
        st.caption("Pipeline completed successfully. Re-run Analyse to process a new source.")
...
if run_btn:
    if not source.strip():
        ...
        steps = [
            ...
            ("rag",        "🧠 Building RAG engine…"),
        ]

        try:
            # st.status() updates its label/state live within a single script
            ...
            with st.status("Starting pipeline…", expanded=True) as status:
                ...
                status.update(label=steps[3][1])
                summary = summarize(transcript)

                status.update(label=steps[4][1])
                action_items = extract_action_items(transcript)
                decisions = extract_key_decisions(transcript)
                questions = extract_questions(transcript)

                status.update(label=steps[5][1])
...
if st.session_state.result:
    ...
    # Chat input — wrapped in a form with clear_on_submit so the box empties
    # after sending, instead of holding onto the last question forever.
